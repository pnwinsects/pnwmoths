...
//   - displayed_as, which is read from the BUILT site because the hand model of the three
...
describe('summarize', () => {
  it('counts by cause, and separately how many appear nowhere at all', () => {
    const rows = buildHiddenImageRows(options({
      ...
      tiled: new Map([['phyllodesma-americana', TILES_A]]),
      thumbnailUse: new Map([
        [thumbnailKey('phyllodesma-americana', 'Phyllodesma americana-A-D.jpg'), new Set(['browse' as const])],
      ]),
    }));
...
describe('buildHiddenImageRows — displayed_as', () => {
  ...
  it('sorts rows shown nowhere ahead of rows still on a thumbnail', () => {
    const rows = buildHiddenImageRows(options({
      ...
      thumbnailUse: new Map([
        [thumbnailKey('aaa-shown', 'Aaa shown-C-D.jpg'), new Set(['browse' as const])],
      ]),
    }));
