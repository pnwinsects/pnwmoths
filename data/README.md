...
| `hidden-images-report.csv` | ~3 800 | **Generated, not hand-edited.** One row per catalogued photograph that reaches no page, with the reason. Written by [`scripts/emit-hidden-images.ts`](../scripts/emit-hidden-images.ts) (`npm run report:hidden-images`) — offline, from the committed data, except `cdn_status`, which reflects `cdn-inventory-report.csv` and is only as fresh as it. Sorted worst-first: `hidden-by-tiles`, `unmatchable-by-tiles`, `family-withheld`, `species-unpublished`, `cdn-missing`, then `superseded-by-tiles` (the bulk, and the least interesting — the same specimen *is* published, as a tile). Advisory: nothing reads it back and it can never fail a build. See [#299](https://github.com/pnwinsects/pnwmoths/issues/299). |
