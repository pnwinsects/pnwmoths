...
| `records-bad-coords.csv` | varies | Records held off the site because their coordinates fall outside the publishing bounds (`RECORD_COORDINATE_BOUNDS` in [`scripts/lib/records-source.ts`](../scripts/lib/records-source.ts)). Written once from the legacy database by [`scripts/recover-clipped-bc-records.ts`](../scripts/recover-clipped-bc-records.ts) and never regenerated: a record leaves the file when it is resolved, its corrected copy appended to `records.csv` or dropped by a curator's ruling. Few are swapped lat/lon; most carry correct coordinates beyond the bounds, which is a coverage question ([#367](https://github.com/pnwinsects/pnwmoths/issues/367)), not bad data. Not built into the site. |
