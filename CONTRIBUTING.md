...
- The source site uses the record type `sight_field_notes`, which is not in the set of valid types accepted by `build-data.js` (`specimen`, `photograph`, `literature`, `field notes`). Importing from the source site requires mapping `sight_field_notes` → `field notes` manually; the validator will silently accept any string that matches valid types only.
- The geographic latitude bound enforced by the build validator (42–52°N) is tighter than the stated PNW region (approximately 42–55°N per `_instructions/ADDING_RECORDS.md` and the custom project instructions; the CONTRIBUTING docs say approximately 42–55°N). Twenty-four British Columbia records for *Phyllodesma americana* are above 52°N and must be excluded, even though the source site considers them part of the BC range. The validator bound and the documentation are inconsistent; one of them needs to be updated.
- Photo license information is not available on the source site; the page only shows "Photograph copyright: [name]" with no explicit open license. The `images.csv` schema requires a `license` field. A convention for handling proprietary or all-rights-reserved photographs needs to be defined.
- The `images.csv` schema does not include fields for image view type (dorsal/ventral) or specimen label (A/B/C). The source site encodes this in the filename suffix (e.g., `A-D` = specimen A, dorsal; `A-V` = specimen A, ventral). This metadata is lost when images are renamed to sequential `01.jpg`, `02.jpg`, etc.
