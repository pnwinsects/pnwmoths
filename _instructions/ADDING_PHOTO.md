...
- Build output: the photo appears on the species page. If the species has high-resolution
  tiles, the account shows the tiles *and* every catalogued photograph no tile covers — so your
  row appears **unless** a tile already shows the same specimen and view (then the tile stands
  in for it), or the row has no `specimen` or `view` (then nothing can tell, and it is hidden).
  `data/hidden-images-report.csv` is what answers it for your row — see [Verify](#verify). The
  rules are in [docs/reference/photo-display-rules.md](../docs/reference/photo-display-rules.md).
...
the species has no high-resolution tiles. If some slides open a **zoomable viewer**, it
does — the tiles come first in the strip and the catalogued photographs no tile covers follow.
...
**Tiled — verify on the species page first.** Your row is in the strip unless a tile already
shows the same specimen and view, in which case the tile stands in for it and the row's
absence from the page is expected, not a failed build. To tell those apart, and to check the
row reached `/browse/`, Identify or another species' "similar species" row:
...
| `unmatchable-by-tiles` | The row has no `specimen` or no `view`, so the account cannot tell whether a tile already shows it and hides it rather than risk a duplicate. | Yours to fix: fill in those two columns of the row you just added. |
...
is your own step to finish, not a question for anyone. A row no tile covers is shown on the
account and has no line in this report at all. (`family-withheld` and `species-unpublished`
