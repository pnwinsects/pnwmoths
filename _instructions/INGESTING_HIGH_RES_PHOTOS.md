...
| `binomial_resolved` | The species binomial the row resolved to; populated for every resolved bucket (`clean-match`, `slug-match`, `resolved-via-synonym`, `resolved-via-determination`), empty otherwise |
| `species_slug` | The species slug the row resolved to; same rule |
