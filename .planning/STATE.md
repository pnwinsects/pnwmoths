...
| 260701-ddi | Unlinked per-species audit CSV — `scripts/emit-species-audit.ts` (`build:species-audit`) emits `_site/species-audit.csv`, one row per `data/species.csv` species with `has_records` (records.csv), `visible` (bug-for-bug `stats.ts` `shown` predicate: withheld-family + unpublished gates), and `in_key` (`key-matrix.json`) flags, joined on `normalizeSlug`. Pure `buildSpeciesAuditRows`/`toCsv` helpers unit-tested; artifact reachable only by direct URL (not linked) | 2026-07-01 | complete ✓ |
| 260702-ega | Contingent character reveal in the Identify key (#97) — new pure `src/_lib/key-dependencies.ts` authors the 15 parent→child rules (empty in Lucid `key.data`), keyed on question string (stigma/ecoregions/precise-size/hindwing-discal/marginal-band); `isQuestionVisible` + fixpoint `pruneHiddenSelections` wired into `pnwm-identify.ts` — dependent questions hidden until parent trigger selected, orphaned child selections pruned on deselect so hidden questions never silently filter. Real-data test guards rule strings vs `key-matrix.json` | 2026-07-02 | complete ✓ |
