...
const reports: CurationReport[] = [
  {
    id: 'hidden-images',
    title: 'Hidden photographs: catalogued images their species account does not show',
    audience: 'curation',
    question: 'Which photographs are catalogued and on the CDN but do not appear on their species account, and where are they still shown?',
    body: 'One row per catalogued photograph that its <strong>species account</strong> does not display, with a direct link to the image so it can be looked at without a login. Read <code>cause</code> first, then <code>displayed_as</code> — the account is not the only place a photograph appears, and the Browse tree, Identify cards and similar-species thumbnails all read the catalogue directly without consulting tiles. A blank <code>displayed_as</code> means the photograph appears nowhere on the site at all, and those rows sort first within each cause. <code>unmatchable-by-tiles</code> is the one to act on: the species has high-resolution tiles, and the row has a blank <code>specimen</code> or a blank <code>view</code> cell, so the account cannot tell whether a tile already shows that photograph and hides it rather than risk a duplicate — fill in the blank cell and it will either appear beside the tiles or be superseded by one. (A view tiles never have, such as <code>lateral</code>, is not unmatchable; the account shows it.) (A row that <em>can</em> be matched and that no tile covers is displayed on the account and is not in this report at all — the <em>Phyllodesma coturnix</em> ventral used to be the example of a photograph that appeared nowhere, before the account started showing such rows beside its tiles.) <code>family-withheld</code> and <code>species-unpublished</code> are whole species that build no page at all. <code>superseded-by-tiles</code>, the bulk of the file, is the uninteresting case kept for completeness: the same specimen and view <em>is</em> published, as a better tile. <code>filename_name_differs</code> flags a photograph filed under one name whose filename carries another — the trace of a rename or a merge, and the fastest way to the interesting rows.',
    regenerated: 'Manually: <code>npm run report:hidden-images</code>. No build required — where each photograph is shown comes from the display index (<code>src/_lib/photo-display-index.ts</code>), which the <code>build:check-display-index</code> gate checks against the emitted HTML on every build. Its <code>cdn_status</code> column is only as fresh as the CDN inventory beside it, so run <code>npm run cdn:inventory</code> first if that column matters.',
