...
const reports: CurationReport[] = [
  {
    ...
    regenerated: 'Manually: <code>npm run report:hidden-images</code>, <strong>after a build</strong> — it reads the emitted <code>_site/</code> to find where each photograph is still shown, rather than predicting it. Its <code>cdn_status</code> column is only as fresh as the CDN inventory beside it, so run <code>npm run cdn:inventory</code> first if that column matters.',
