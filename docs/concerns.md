...
| Link check never verifies an image URL | `exclude` drops every `.jpg/.png/.webp/…` URL, so no image has ever been checked by CI — which is why 83 rows referencing files absent from the CDN went unnoticed ([#232](https://github.com/pnwinsects/pnwmoths/issues/232)). `check-derivatives.ts` now closes most of this offline: it proves every image reachable from a built page has its derivatives on the CDN, and `npm run cdn:inventory` checks the same claim against the zone itself on demand ([ADR 0036](adr/0036-cdn-inventory-by-accountability.md)). What remains unverified per-build is the handful of non-derivative image URLs — plates, site images, the 1500w hero slot. | `lychee.toml:26`, `scripts/check-derivatives.ts` | ACCEPTED (residual gap is small; a network check would fail builds on a blip) |
...
| ~~83 orphan rows in `data/images.csv`~~ | **Resolved 2026-08-24 (#232, C-027).** All 83 originals were still on `dev.pnwmoths.biol.wwu.edu` under `/media/moths/`, which serves them without authentication — recovered and uploaded. The count here said 27 species; it was 24. Ten resolved only under their pre-C-023 filenames, which is the concrete cost that [ADR 0038](adr/0038-photo-identity-is-data-not-filename.md) now forbids repeating. The Geometridae embargo (#48) no longer has a missing-photo blocker. | [#232](https://github.com/pnwinsects/pnwmoths/issues/232), `data/images.csv` | RESOLVED |

## Code debt

| Item | What / why it matters | Pointer | Status |
|------|-----------------------|---------|--------|
| No owner for photo display selection | `data/images.csv` has **five display call sites** — the species account, the Browse species card, the Browse genus/tribe/family strips, Identify cards and similar-species thumbnails — each with its own selection rule, across four files in three languages. **Only the account knows high-resolution tiles exist**, so tiling a species removes its catalogued photographs from its own page while leaving them on `/browse/` and Identify. There is no module to ask "where does this photograph appear", which is why [`scanBuiltSite()`](../scripts/emit-hidden-images.ts) answers it by grepping the emitted HTML — honest and verified, but a data report that needs a completed build, and a workaround for a missing seam. Three successive attempts to model the rules from source were each wrong. | [#338](https://github.com/pnwinsects/pnwmoths/issues/338), [#337](https://github.com/pnwinsects/pnwmoths/issues/337), `src/_data/taxon.ts`, `scripts/build-key.ts`, `src/species/species.njk` | ACTIONABLE |
...
|------|------|---------|--------|
| 22 specimen letters claimed by two photographs | Each is an earlier redetermination that kept the old filename, so one species holds two different photographs both labelled specimen A (`Grammia nevadensis-A-D.jpg` and `Grammia incorrupta-A-D.jpg` under `apantesis-nevadensis`). Harmless while neither is tiled; the moment one is, the account can show the wrong moth — that is exactly how #330/#336 happened. `check-photo-determinations.ts` check D **ratchets** them: the 22 that predate the check are listed in `KNOWN_COLLISIONS` and stay advisory, because each needs the curator and failing the build on a backlog of taxonomic questions would stop all work — but a collision that is not on that list is fatal, and so is a list entry that no longer collides. The debt does not block work and cannot grow silently. | [#341](https://github.com/pnwinsects/pnwmoths/issues/341), `scripts/check-photo-determinations.ts` | ACTIONABLE (curator decision) |
