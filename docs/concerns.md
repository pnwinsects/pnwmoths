...
| Item | What / why it matters | Pointer | Status |
|------|-----------------------|---------|--------|
| ~~No owner for photo display selection~~ | **Resolved 2026-08-25** by [ADR 0040](adr/0040-photo-display-module.md) ([#338](https://github.com/pnwinsects/pnwmoths/issues/338)). The seven rules over `data/images.csv` now live in one module, [`src/_lib/photo-display.ts`](../src/_lib/photo-display.ts), with `TILE_POLICY` stating per surface what tiles do — the asymmetry that let a tiled species keep publishing its photographs everywhere except its own page. "Where does this photograph appear" is derived from those pickers by [`src/_lib/photo-display-index.ts`](../src/_lib/photo-display-index.ts) rather than grepping the built site, so [`emit-hidden-images.ts`](../scripts/emit-hidden-images.ts) no longer needs a completed build; [`check-display-index.ts`](../scripts/check-display-index.ts) fails `build:site` if that index and the emitted HTML disagree about any of the 4,034 photographs. | [ADR 0040](adr/0040-photo-display-module.md), [docs/reference/photo-display-rules.md](reference/photo-display-rules.md) | RESOLVED |
