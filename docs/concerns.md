...
|------|------|---------|--------|
| 22 specimen letters claimed by two photographs | Each is an earlier redetermination that kept the old filename, so one species holds two different photographs both labelled specimen A (`Grammia nevadensis-A-D.jpg` and `Grammia incorrupta-A-D.jpg` under `apantesis-nevadensis`). Harmless while neither is tiled; the moment one is, the account can show the wrong moth — that is exactly how #330/#336 happened. `check-photo-determinations.ts` check D reports them and is deliberately advisory: each needs the curator, and failing the build on a backlog of taxonomic questions would stop all work. | [#341](https://github.com/pnwinsects/pnwmoths/issues/341), `scripts/check-photo-determinations.ts` | ACTIONABLE (curator decision) |
