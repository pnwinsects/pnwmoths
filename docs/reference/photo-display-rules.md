...
| Surface | Picker | Rule |
|---|---|---|
| Species account carousel | `pickAccountPhotos` | Without high-res tiles: **every** row for the species, in `weight` order. With tiles: every tile, then every row whose **specimen and view no tile covers**, in `weight` order ([ADR 0041](../adr/0041-account-shows-photos-tiles-do-not-cover.md)). A row with a blank `specimen` or a blank `view` cannot be compared and is not shown. A tile's caption carries the label data (locality, date, collector) of the row it supersedes — only when exactly one row has that specimen and view; two rows under one letter are two moths (#341), so the tile gets no label rather than a guess ([ADR 0046](../adr/0046-tile-caption-from-sole-covered-row.md)). |
...
- **The account is the only surface that compares a photograph against the tiles.** A tile
  supersedes every catalogued row of the same specimen and view (there can be more than one —
  [#341](https://github.com/pnwinsects/pnwmoths/issues/341)); every other row still renders
  beside the tiles, including any lateral or head view, which no tile could ever cover.
  ...
  `tileOutcome()`, and the hidden-images report classifies with the same function so the two
  cannot disagree; a row with a blank `specimen` or a blank `view` is `unmatchable` and hidden
