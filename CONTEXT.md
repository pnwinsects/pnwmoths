...
- **slug** — the canonical species key: `(genus + '-' + species).toLowerCase()`, alphanumeric and hyphens only. It is both the URL segment (`/species/{slug}/`) and the foreign key across every CSV that references a species. Slugs are self-documenting for non-technical contributors; numeric IDs are avoided. Every place a species is referenced by slug is declared in `RELATIONS` in [`scripts/check-referential-integrity.ts`](scripts/check-referential-integrity.ts) and checked before the build, **strictly** — a reference that resolves only after normalization joins to nothing at runtime and is reported as a near-miss. **Adding a data file that names species means adding a line there**; a test enforces that for everything under `data/`, and sources elsewhere are declared by hand ([ADR 0033](docs/adr/0033-referential-integrity-gate.md)).
...
- **SpeciesImage** — a photo in `data/images.csv`: keyed to a species by slug, with photographer credit and a **`weight`** — the integer, low first, that orders a species' photographs and therefore selects them. There is no flag that overrides it: a curator promotes a photograph by giving it the lowest `weight` for its species. (A `navigational` column once claimed that role; it was empty in all 4,034 rows and is gone — [ADR 0039](docs/adr/0039-photo-display-selection-by-weight.md).)
- **navigation image** — whichever photograph a surface picks to stand for a species: the Browse thumbnail, a genus/tribe/family strip image, an Identify card. **Derived from `weight`, never declared** — and derived differently by each of the seven display rules, which is why they are inventoried in one place, [docs/reference/photo-display-rules.md](docs/reference/photo-display-rules.md). Notably: Browse excludes `ventral` views and nothing else does; the genus strip takes four images across a whole genus rather than one per species; and only the species account knows that high-res tiles replace the catalogued photographs. No module owns the question ([#338](https://github.com/pnwinsects/pnwmoths/issues/338)).
...
- **Referential-integrity exceptions** — `data/referential-integrity-exceptions.csv`. The known orphan slugs, each naming the issue that will resolve it. A ratchet, not a mute switch: a new orphan fails the build, and so does an entry whose fault has been fixed ([ADR 0033](docs/adr/0033-referential-integrity-gate.md)).
