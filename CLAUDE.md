...
- **[docs/reference/](docs/reference/)** — provenance of the legacy data (original WWU site, reference MySQL DB, Lucid3 key source, subfamily-in-CMS-paths) and the [photo display rules](docs/reference/photo-display-rules.md) — which photograph each surface shows, until [#338](https://github.com/pnwinsects/pnwmoths/issues/338) gives them an owner.
...
- **Photo display selection lives in one module, and the index derived from it is gated** — the seven rules over `data/images.csv` are pickers in [`src/_lib/photo-display.ts`](src/_lib/photo-display.ts) (SQL fragments beside their in-memory twins, because two consumers select in DuckDB); *where* a photograph appears is derived from them by `photo-display-index.ts` and checked against the emitted HTML by `check-display-index.ts` on every build. **A new surface means a picker, an index branch, and a row in [docs/reference/photo-display-rules.md](docs/reference/photo-display-rules.md)** ([ADR 0040](docs/adr/0040-photo-display-module.md)).
